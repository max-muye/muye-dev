...
<body>
  <main>
    <header>
      ...
      <div class="header-side">
        <button class="secondary-button" id="lang-toggle" type="button">中</button>
  ...
  <script>
    ...
    let lang=localStorage.getItem("localtalk-lang")||"en";let currentMessages=[];let mySenderKey="";let quote=null;let publicBanned=false;let publicBanReason="";let uploadBusy=false;let sendBusy=false;let filePickUntil=0;
    const messages=document.querySelector("#messages"),form=document.querySelector("#form"),input=document.querySelector("#text"),file=document.querySelector("#file"),status=document.querySelector("#status"),themeToggle=document.querySelector("#theme-toggle"),langToggle=document.querySelector("#lang-toggle"),notifyToggle=document.querySelector("#notify-toggle"),quoteBar=document.querySelector("#quote-bar"),quoteText=document.querySelector("#quote-text"),quoteClear=document.querySelector("#quote-clear");
    ...
    function syncText(){document.querySelectorAll("[data-i18n]").forEach(el=>el.textContent=t(el.dataset.i18n));document.querySelector("#public-disclaimer-title").textContent=t("disclaimerTitle");document.querySelector("#public-disclaimer").textContent=t("disclaimer");input.placeholder=t("message");langToggle.textContent=lang==="en"?"中":"EN";themeToggle.textContent=root.dataset.theme==="dark"?t("light"):t("dark");syncNotify()}
    ...
    langToggle.addEventListener("click",()=>{lang=lang==="en"?"zh":"en";localStorage.setItem("localtalk-lang",lang);syncText();status.textContent=t("online")});
...
<body>
  <main>
    <header><div><h1 id="admin-title">Local Talk Admin / 本地聊天管理</h1><div class="muted" id="admin-sub">manage messages and bans / 管理消息和封禁</div></div><div class="actions"><button class="secondary" id="lang-toggle" type="button">EN / 中</button><a class="secondary" href="/" id="home">Home / 主页</a><a class="secondary" href="/talk/" id="back">Back / 返回</a></div></header>
    <section>
  ...
  <script>
    ...
    roomCreate.onclick=async()=>{const room=roomName.value.trim();if(!room)return;const r=await adminFetch("/admin/create-room",{room});notice.textContent=r.ok?t("roomCreated"):t("roomFailed");if(r.ok)roomName.value=""};
    ...
    langToggle.onclick=()=>{lang=lang==="en"?"zh":"en";localStorage.setItem("localtalk-lang",lang);syncText();load()};
    syncText();
  </script>
</body>
</html>`;
...
<body>
  ...
  <script>
    ...
    const lang=localStorage.getItem("localtalk-lang")||"en";
